...
Spath = '../Dataset/TOSCA/SGP_dataset_off/isometric/null/';

names = {'cat', 'centaur', 'david', 'dog', 'horse', 'michael', 'victoria', 'wolf'};

for nameID = 1:8
    
    name = names{nameID};
    
    finf = dir([Spath,name,'/*.off']);
    
    finf2 = dir([Spath,name,'/*.gt']);
    
    % pairs =  nchoosek(1:length(finf),2);
    % save(['../Results/TOSCA/',name,'.mat'],'pairs');
    load(['../Results/TOSCA/',name,'.mat']);
    ...
    for methodIndex = 15
        ...
        for i=1:size(pairs,1)
            ...
            GT1 = load(finf2(pairs(i,1)).name);
            GT2 = load(finf2(pairs(i,2)).name);
            ...
            switch methodIndex
                case 1
                    %% LPO - ours
                    Nf = 5; max_iters = 8; lambda = 0.2;
                    [T, C] = MWP_L_v6(S1, S2, k, T0, Nf, max_iters,lambda);
                ...
                case 11
                    %% CPD - regularized pointwise map recovery
                    f = 5;
                    num_iters = 4;
                    [~, C] = MWP2(S1, S2, 500, T0, f, num_iters);
                    
                    M.VERT = S1.surface.VERT; M.TRIV = S1.surface.TRIV; M.n = S1.nv; M.m = S1.nf; M.S = S1.Gamma; M.evecs = S1.evecs; M.evals = S1.evals;
                    N.VERT = S2.surface.VERT; N.TRIV = S2.surface.TRIV; N.n = S2.nv; N.m = S2.nf; N.S = S2.Gamma; N.evecs = S2.evecs; N.evals = S2.evals;
                    T = run_cpd(M, N, C, 10);
        ...
        save(['../Results/TOSCA/',name,'-',methods{methodIndex},'_detailTime.mat'],'Error','Time');
    end
end